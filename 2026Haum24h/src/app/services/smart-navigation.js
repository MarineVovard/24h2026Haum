export class SmartNavigation {
     ...
     */
    static getNextMoveAStar(current, target, obstacles, maxSpeed, gridResolution = 2.0, obstacleRadius = 5.0) {
    ...
    static distancePointToSegment(p, v, w) {
        const l2 = Math.pow((w[0] - v[0]), 2) + Math.pow((w[1] - v[1]), 2); // Longueur au carré
        if (l2 === 0)
            return this.distance(p, v); // Segment = 1 point
